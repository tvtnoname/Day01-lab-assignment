...
def compare_models(prompt: str) -> dict:
    ...
    # Ước tính chi phí: chuyển số từ sang số token rồi nhân giá
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Chỉ giữ lại 3 lượt hội thoại gần nhất
        history = history[-3:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        row = [
            truncate(r.get("prompt", ""), 42).ljust(42),
            truncate(r.get("gpt4o_response", ""), 42).ljust(42),
            truncate(r.get("mini_response", ""), 42).ljust(42),
            f"{r.get('gpt4o_latency', 0):.3f}s".ljust(16),
            f"{r.get('mini_latency', 0):.3f}s".ljust(16),
        ]
